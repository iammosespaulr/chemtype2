...
BOND_PATHS = ['train/single/', 'train/double/',
              'train/triple/', 'train/dashed/', 'train/wedge/']
...
def template_match(template, img, min_scale=0.3, max_scale=1.0, n_scales=15, threshold=0.6):
    ...
    im = cv2.copyMakeBorder(im, BORDER, BORDER, BORDER,
                            BORDER, cv2.BORDER_CONSTANT, 0)
...
def all_template_match(templates, template_names, img, tol=0.6, display=False):
    ...
    if display:
        ...
        im = cv2.copyMakeBorder(
            im, BORDER, BORDER, BORDER, BORDER, cv2.BORDER_CONSTANT, 0)
        im = cv2.GaussianBlur(im, (LINE_WIDTH/2, LINE_WIDTH/2), LINE_WIDTH/2)
        ...
        for key in list(template_dict.keys()):
            if len(template_dict[key]) != 0:
                ...
                for box in template_dict[key]:
                    cv2.rectangle(
                        im, (box[0], box[2]), (box[1], box[3]), color=color, thickness=5)
        plt.imshow(im)
        plt.ion()
        plt.show()
...
def all_template_match_all_images(templates, template_names, path, tol=0.6, display=False):
    ...
    for i, image in enumerate(os.listdir(path)):
        ...
        template_dict, acc = all_template_match(
            templates, template_names, full_name, tol=tol, display=display)
        correct += acc
...
def reimplement_polygon(img, template_dict, min_dist=50):
    ...
    for contour in contours:
        ...
        approxCurve = cv2.approxPolyDP(
            contour_array, LINE_WIDTH*np.sqrt(2), True)
        for vertex in approxCurve:
            ...
            if len(dists) == 0:
                ...
                if dists[min_dist_cluster] < min_dist:
                    node_centers[min_dist_cluster] = get_new_center(
                        node_centers[min_dist_cluster], point, node_counts[min_dist_cluster])
                else:
                    node_names.append('C' + str(node_idx))
                    node_centers.append(point)
                    node_counts.append(1)
                    node_idx += 1
            cv2.rectangle(display_im, (point[1]-4, point[0]-4),
                          (point[1]+4, point[0]+4), color=[0, 0, 255], thickness=-1)
    ...
    for point in node_centers:
        ...
        cv2.rectangle(display_im, (point[1]-4, point[0]-4),
                      (point[1]+4, point[0]+4), color=[255, 0, 0], thickness=-1)
    print(node_names)
    plt.imshow(display_im)
    plt.title("Centers")
    plt.ion()
...
def corner_detector(img, template_dict, max_corners=20, display=True, rect_w=6):
    ...
    corners = cv2.goodFeaturesToTrack(
        im, 20, 0.0001, 35, blockSize=40, useHarrisDetector=1, k=0.04)
    im = cv2.cvtColor(im, cv2.COLOR_GRAY2RGB)
    if corners is None:
        return 0
    for corner in corners:
        ...
        cv2.rectangle(im, (corner_y-rect_w/2, corner_x-rect_w/2), (corner_y +
                                                                   rect_w/2, corner_x+rect_w/2), color=[255, 0, 0], thickness=-1)
...
def detect_bond_between_corners(im, corner1, corner2, bbox_width, angle_tol, hough_tol=10, window_spacing=15):
    ...
    for degree in np.linspace(0, 1, np.linalg.norm(v)/window_spacing, endpoint=False):
        ...
        for i, point in enumerate(point_list):
            ...
            cv2.line(display_im, tuple(point1), tuple(
                point2), color=[255, 0, 0], thickness=2)

        lines = cv2.HoughLines(new_im, 1, np.pi/180, hough_tol)
        line_detected = False
        try:
            original_theta = np.arctan(
                (corner2[1]-corner1[1])/(corner2[0]-corner1[0])) + np.pi/2
...
def detect_bonds(img, template_dict, corner_file, bbox_width=40, angle_tol=1):
    ...
    for corner1 in corners:
        dists = [np.linalg.norm(np.array(corner2)-np.array(corner1))
        ...
        for idx in dist_sort[0:max_idx]:
            ...
            if (corner2, corner1) in checked:
                ...
                v = np.array([corner2[0] - corner1[0],
                              corner2[1] - corner1[1]])
                v_orth = np.array(
                    [corner1[1]-corner2[1], corner2[0] - corner1[0]])
                v_orth_norm = v_orth / np.linalg.norm(v_orth)
                corner1_vec = np.array(corner1)
                corner2_vec = np.array(corner2)
            ...
            if flag == 1:
                continue
            line_detected = detect_bond_between_corners(
                im, corner1, corner2, bbox_width=bbox_width, angle_tol=angle_tol)
            if line_detected:
                edges.append((corner1, corner2))
                cv2.line(display_im, tuple(corner1),
                         tuple(corner2), (0, 0, 255), 2)
    plt.imshow(display_im)
    plt.ion()
    plt.show()
    c = input("Correct? (y/n) --> ")
...
def preprocess_training(image_dict, size=(40, 100), norm_width=40):
    ...
    for bond_type in list(image_dict.keys()):
        ...
        for img in imgs:
            ...
            im = cv2.copyMakeBorder(
                im, 0, 0, border, border, cv2.BORDER_CONSTANT, 0)
...
def get_bonds(line_segments, im):
    ...
    for line_segment in line_segments:
        ...
        cv2.rectangle(display, (pt1y-3, pt1x-3), (pt1y+3, pt1x+3),
                      color=[255, 0, 0], thickness=-1)
        cv2.rectangle(display, (pt2y-3, pt2x-3), (pt2y+3, pt2x+3),
                      color=[255, 0, 0], thickness=-1)
        # plt.imshow(display)
        # plt.show()
        ...
        cv2.rectangle(translated_display, (pt1y-3, pt1x-3),
                      (pt1y+3, pt1x+3), color=[255, 0, 0], thickness=-1)
        cv2.rectangle(translated_display, (pt2y-3, pt2x-3),
                      (pt2y+3, pt2x+3), color=[255, 0, 0], thickness=-1)
...
def classify_bonds(edge_file, img, classifier, label_dict, template_dict_file, rect_w=6, empty_thresh=0.3):
    ...
    for i, line_segment in enumerate(edges):
        ...
        if color == 'none':
            continue
        cv2.line(color_im, line_segment[0],
                 line_segment[1], color, thickness=5)
    for key in list(template_dict.keys()):
        if len(template_dict[key]) != 0:
            color = COLOR_DICT_OCR[key]
            for box in template_dict[key]:
                cv2.rectangle(
                    color_im, (box[0], box[2]), (box[1], box[3]), color=color, thickness=5)
    plt.imshow(color_im)
    plt.ion()
    plt.show()
    c = input("Correct? (y/n) --> ")
...
processed_dict = preprocess_training(BOND_TRAINING_DICT)
classifier_svm, label_dict_svm = train_classifier(
    processed_dict, train_split=1)
classifier_decision, label_dict_decision = train_classifier(
    processed_dict, train_split=1)
classifier_logistic, label_dict_logistic = train_classifier(
    processed_dict, train_split=1)
...
for path in PATHS:
    ...
    for image in os.listdir(path):
        if image[len(image)-4:len(image)] != '.png':
            continue
        try:
            corr, n_corr = classify_bonds('pickles/' + image[0:11] + '_edges.pickle', path+image,
                                          classifier_svm, label_dict_svm, 'pickles/' + image[0:11] + '_tol_eq_0.77_template_bb.pickle')
            corr_mol += corr
            corr_edge += n_corr
            total += 1
        except IOError:
            pass
        try:
            corr, n_corr = classify_bonds('pickles/' + image[0:10] + '__edges.pickle', path+image,
                                          classifier_svm, label_dict_svm, 'pickles/' + image[0:10] + '_tol_eq_0.77_template_bb.pickle')
            corr_mol += corr
            corr_edge += n_corr
            total += 1
        except IOError:
